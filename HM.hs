...
type_ ctx tm ty =
  ...
  ( fresh $ \(x,tm0,tx,e) -> do{ tm `eq` let_ x tm0 e
                               ; type_ ctx tm0 tx
...
ex9 = tst $ fresh $ \ty -> do{ type_ nil tm_id ty; expand' ty }

ex10 = tst $ fresh $ \ty -> do{ type_ nil (let_ f tm_id $ var f `app` var f) ty; expand' ty } where f = A"f"
